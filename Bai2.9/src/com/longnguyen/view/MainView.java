...
public final class MainView extends JFrame {
	...
	JButton btnSaveEmp, btnDeleteEmp, btnNewEmp, btnSaveToFile, btnSave, btnDelete, btnNew, btnSavaToFile;
	...
	private void addEvents() {
		...
		btnSavaToFile.addActionListener(new ActionListener() {
	...
	protected void xuLyLuuFile() {
		...
		try {
			...
			for (DepartmentModel departmentModel : dsNhom) {
				S = departmentModel.getMaPB() + "-" + departmentModel.getTenPB() + ":";
				pWriter.println(S);
				for (EmployeeModel employeeModel : departmentModel.getNhanVien()) {
					S = employeeModel.getMaNV() + "," + employeeModel.getTenNV() + "," + employeeModel.getLuongNV();
					pWriter.println(S);
				}
			}
			JOptionPane.showConfirmDialog(null, "File đã lưu xuống " + fileName);
		} catch (Exception e) {
	...
	private void addControls() {
		...
		btnSavaToFile = new JButton("Save To File");
		JPanel pnBtnSaveToFile = new JPanel();
		pnBtnSaveToFile.add(btnSavaToFile);
		pnBtnSaveToFile.setPreferredSize(new Dimension(50, 50));
		pnLeft.add(pnBtnSaveToFile);

		con.setLayout(new BorderLayout());
		pnLeft.setPreferredSize(new Dimension(400, 600));
